# OCOR Architectural Design Document v1.3 — APPROVED COMPOSITE BASELINE

## Document control

| Field | Value |
|---|---|
| Identifier | `OCOR-ADD-1.3-APPROVED` |
| Status | **APPROVED ARCHITECTURAL BASELINE — DEC-208** |
| Effective date | 2026-09-01 |
| Authority | Luca Lillo, Product Owner and repository owner, acting as Architecture Review Authority |
| Human authorization | Explicit instruction: «procedi con il passaggio autoritativo» |
| Change set | `CC-FULL-GOVERNED-AGENT-MEMORY` |
| Supersedes | `OCOR-ADD-1.2` only through the approved amendment in Part II |
| Evidence fence | `E1=0`, `E2=0`, zero global `Verified`; `E1_runtime_slice=PRESENT` remains confined to `DEC-207` |
| Runtime disposition | **FULL-MEMORY RUNTIME NO-GO pending FGM-01–FGM-20** |

## Composition and precedence

This file is the complete content-addressed ADD v1.3 baseline. Part I incorporates the
approved ADD v1.2 baseline verbatim for auditability. Part II contains the amendment
approved by `DEC-208`. If a Part II clause explicitly replaces or adds to a Part I
clause, Part II prevails. All other Part I clauses remain effective. No external
candidate document is needed to interpret this baseline.

---

# Part I — ADD v1.2 incorporated verbatim
# OCOR Architectural Design Document v1.2 — APPROVED BASELINE

## Approval control

| Field | Approved value |
|---|---|
| Baseline identifier | `OCOR-ADD-1.2` |
| Status | **APPROVED BASELINE — WITH ACCEPTED NON-BLOCKING VALIDATION LIMITATIONS** |
| Effective date | 2026-08-30 |
| Approval authority | Luca Lillo, Product Owner and repository owner, acting as Architecture Review Authority |
| Approval evidence | Explicit user instruction of 2026-08-30: «puoi dichiararlo approvato» |
| Incorporated candidate | `reports/OCOR_Architectural_Design_Document_v1.2_Candidate.md` |
| Candidate SHA-256 | `4f84249b86150a0aa0ef5bf7fcc1a5c99388651e8aae132720dd784883b0426f` |
| Decision record | `ARA_DECISION_RECORD_v1.1.md` |
| Approved decisions | `DEC-197`–`DEC-206` |
| Evidence state | `E1=0`, `E2=0`, zero requirements `Verified` — unchanged |

## Normative supersession rules

1. This approval control and `ARA_DECISION_RECORD_v1.1.md` supersede candidate-status statements in the incorporated text. Occurrences of `PROPOSED — AWAITING CHANGE CONTROL`, `Proposed — awaiting change control`, “approval not yet registered”, and equivalent candidate-only wording are retained below solely as historical provenance.
2. `DRAFT-A`–`DRAFT-I` are ratified through `DEC-197`–`DEC-204` as mapped in the ARA record; no draft slug remains an independent source of authority.
3. `FR-095` is approved as P0/MVP, not PoC; `ELM-070` remains deferred outside the PoC. `DEC-205` supersedes only the conflicting release clause of `DEC-103`.
4. The local atomic commit of aggregate revision, canonical commit, idempotency binding and outbox is a hard invariant. No fallback is pre-approved; backend non-conformance is `NO-GO` and requires new change control.
5. The ten approved change sets are indivisible where the dependency DAG or atomic-set rule says so. Partial interpretations that weaken GCS, Human Gate, emission fencing, marking, branch isolation or adjudication are invalid.
6. Open issues, assumptions, risks, deferred capabilities and candidate technologies keep their prior status unless explicitly changed by `DEC-197`–`DEC-206`.
7. Approval is architectural only. It does not establish implementation, performance, security, HA, portability, production readiness or equivalence claims.

## Accepted non-blocking validation limitations

- The official OpenAPI 3.1 semantic validator remains `NOT_EXECUTED`; structural validation and reference resolution passed. Execution or a formal tool waiver is required before the first externally released interface baseline.
- The post-remediation approval-readiness verifier passed 18/18 checks. The earlier 105/105 conformance, 29/29 semantic and 12/12 release-gate results apply to invariant-bearing blocks unchanged by remediation. A full harness rerun on this approved package is required before implementation-gate promotion, not as a condition of architectural approval.

---

# Incorporated normative text

The complete candidate text below is incorporated by value. Where its historical document-control statements conflict with the approval control above, the approval control and ARA record prevail.
...
| Identificativo | `OCOR-ADD-1.2-CANDIDATE` |
| Versione | `v1.2 Candidate` |
| Data | 30 agosto 2026 |
| Derivazione | ADD v1.1, SHA-256 `3e7532c7101e80c74b511cedc56a0563ca0938a3fe259911a91245cb19dedc7f` |
| Sostituisce | Nessuna baseline approvata: è una candidata separata; la v1.1 resta immutabile e ricostruibile |
| Origine della revisione | `OCOR ADD v1.1 — Independent Delta Review`, finding `DRF-001`–`DRF-015`, più red-team v1.2 |
| Natura della revisione | Remediation candidata completa. Le modifiche che richiedono authority sono `PROPOSED — AWAITING CHANGE CONTROL`; nessuna capability attivata, nessun requisito sorgente aggiunto/rimosso, nessun `OI-*`/`ASM-*`/`RSK-*` chiuso, stato probatorio invariato |
...
| Stato documentale | `PROPOSED — AWAITING CHANGE CONTROL` |
| Stato delle modifiche v1.2 | Soluzioni candidate univoche e fail-closed; Amendment Log §8; nessuna modifica entra nella baseline senza authority competente |
...
**Nota sulla revisione v1.2 candidata.** Questa versione corregge i difetti interni confermati della v1.1 e propone, senza approvarle, le decisioni necessarie per GCS, mutazione canonica, safety di emissione, marking, adjudication e change control. `DRAFT-A`–`DRAFT-I` restano bozze non approvate e questa candidata non assegna alcun nuovo identificativo decisionale oltre la baseline. La proposta conservativa su `FR-095` mantiene `ELM-070` differita e richiede la riclassificazione del requisito fuori dal PoC: fino all'approvazione dei registri la discrepanza è un authority blocker, non una modifica di baseline compiuta. L'Amendment Log completo è in §8.
...
Ogni chiamata, evento, cache entry, embedding, log, explanation e artefatto derivato è associato a una e una sola istanza del record canonico **`GovernedContext` v1.2**:
...
`GovernedContext` è chiuso: cardinalità esattamente uno per tutti i campi, salvo `compartments` e `actor_chain` che sono array non vuoti, ordinati dove semanticamente rilevante e senza duplicati. `classification_marking_ref` è sempre la forma canonica `urn:sha256:<marking_digest>`. `tenant_id`, `organization_id`, `domain_id`, `compartments`, `purpose` e `correlation_id` possono provenire dal `BODY` soltanto come valori dichiarati da confrontare con i binding autorizzati. `effective_principal_id` è sempre `DERIVED_FROM_VERIFIED_BINDING` dal Principal autenticato del trasporto; `actor_chain` è sempre derivata dalla Delegation chain verificata. `ontology_release_digest` e `policy_bundle_digest` sono assegnati dal boundary di ammissione e firmati nel binding. Nessun Principal, Goal, prompt, payload o producer dichiarato nel body concede Authority.

Il boundary costruisce atomicamente `GovernedContext`, ne calcola `governed_context_digest` e lo lega all'artefatto. Un valore body incompatibile con il binding autenticato non è corretto silenziosamente: all'ingress esterno produce `QUARANTINED{reason=PRINCIPAL_BINDING_MISMATCH}` e Audit Record; sulle superfici interne o mutative produce `DENY{reason=GOVERNED_CONTEXT_MISMATCH}` prima di cache lookup, query, dispatch o commit. Un campo mancante, non risolvibile, sconosciuto o ambiguo è fail-closed. L'equivalenza fra forme serializzate richiede uguaglianza byte-canonica dei valori scalari, equivalenza insiemistica dei compartimenti dopo canonical sort e coincidenza dei digest; alias o coercizioni non dichiarate sono vietati. La tabella di realizzazione è in §3.0.1.

La chiave di isolamento è la tupla `(tenant_id, organization_id, domain_id, compartments, purpose, classification_marking_ref, ontology_release_digest, policy_bundle_digest)`. Cache e indici non sono condivisi fra tuple incompatibili; code, topic e storage prefix sono compartment-scoped; una query federata produce soltanto l'intersezione autorizzata. Il marking derivato è il `conservative_join` degli input e delle obligation. Marking sconosciuti o non confrontabili producono `DENY`/`QUARANTINE` e `DENY_EXPORT`; ogni abbassamento crea un distinto artefatto di sanitizzazione/declassificazione con authority, Approval e audit.
...
| Proposta di change control | `FR-095`: riclassificazione da P0/PoC a P0/MVP, fuori dal PoC, mantenendo `ELM-070` differita — `PROPOSED — AWAITING CHANGE CONTROL` |
...
Ogni riga è un candidato a `NO-GO` del release profile che vi dipende. I fallback `BA-02`–`BA-08` restano sostituzioni confinate se preservano i contratti; `BA-01` è esplicitamente esclusa da questa regola perché cambia l'invariante di atomicità e richiede change control architetturale.
...
Il `GovernedContext` di §1.4 è normativo per ogni superficie. `BODY` indica un valore serializzato e validato; `TRANSPORT` un valore autenticato non controllabile dal body; `DERIVED` una proiezione deterministica da binding verificato. Ogni riga produce lo stesso record canonico e lo stesso digest.

| Superficie | Forma serializzata / mapping | Origine Principal | Enforcement point | Mismatch / campo assente |
|---|---|---|---|---|
| Ingestion | `context.*`; marking inline risolto a `classification_marking_ref`; `integrity.source_producer_principal_ref` è soltanto source assertion | `TRANSPORT` → `effective_principal_id` | `C5` admission boundary | external mismatch → `QUARANTINED`; incomplete → `REJECTED` |
| Query | `QueryContext` usa gli undici nomi canonici; nessun Principal nel body | `TRANSPORT` | `C2` auth/policy interceptor, prima di cache/router | `DENY` senza lookup |
| Function/model invocation | `InvocationContext` usa gli undici nomi canonici | `TRANSPORT` | `C2`/registry invocation interceptor | `DENY` |
| MCP | `governed_context_ref` risolve a record immutabile; input schema non può sovrascriverlo | `TRANSPORT` | `C8` capability broker e `C2` tool boundary | `CAPABILITY_DENIED` |
| Action Type | `governed_context_requirements` dichiara tutti i campi e vieta authority dal body | `DERIVED` | `C1` contract compiler | contract `RELEASE_REJECTED` |
| Action Instance/Command | `governed_context` + digest nel `GatePackage`; command porta lo stesso digest | `TRANSPORT`/`DERIVED` | `C6` create, gate ed emission fence | `INVALIDATED` prima dell'invio |
| Event Subscription | filter context e delivery context usano `governed_context_ref`; consumer non può ampliare scope | `TRANSPORT`/`DERIVED` | `C5` subscribe e per-delivery policy | `POLICY_DENIED`/quarantine |
| Event delivery/replay | admitted envelope porta record/digest; replay ricostruisce lo stesso context e richiede window | `DERIVED` | `C5` delivery/replay controller | `REPLAY_DENIED`/quarantine |
| Scenario | `ScenarioRunSpec.governed_context`; branch key include digest | `TRANSPORT`/`DERIVED` | `C7` scenario admission | `SCENARIO_ISOLATION_VIOLATION` |
| Agent handoff | `HandoffEnvelope.governed_context`; sender dichiarato è confrontato col transport Principal | `TRANSPORT` | `C8` handoff broker | `CAPABILITY_DENIED` + audit |
| Security context | proiezione valutata di `GovernedContext` e marking; non modello alternativo | `DERIVED` | PDP/PEP | `DENY` |
| Observability | log/trace/metric portano `governed_context_digest`; accesso risolve il record con policy | `DERIVED` | telemetry admission + query | drop sicuro o quarantine; mai log non governato |

La validazione di equivalenza è identica su tutte le superfici. L'assenza del Principal dal body Query/Invocation è deliberata; l'assenza del `GovernedContext` risultante nell'Audit Record o nel GatePackage è sempre un difetto bloccante.
...
| `MarkingSchemeDefinition` | `scheme_id`, `version`, `levels[1..*]`, `top`, `bottom`, `dominance_relation`, `family_definitions[5]`, `unknown_value_behavior`, `incomparable_behavior`, `declassification_authority_ref` | `dominance_relation` è riflessiva, antisimmetrica e transitiva; ogni coppia supportata ha `join` e `meet` univoci. Le cinque famiglie obbligatorie sono `mandatory_markings`, `caveats`, `dissemination_controls`, `permitted_purposes`, `handling_instructions`; ciascuna dichiara vocabolario chiuso, `kind=RESTRICTION|PERMISSION`, `join_operator=UNION|INTERSECTION`, `meet_operator` duale e neutral element. `unknown_value_behavior=DENY`; incomparabili ⇒ `DENY|MOST_RESTRICTIVE` deterministico; nessun default implicito |
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "urn:ocor:schema:canonical-ingestion-envelope:1.2",
  ...
  "properties": {
    ...
    "effective_principal_id": {"$ref": "#/$defs/stableId"},
    "actor_chain": {
      "type": "array",
      "minItems": 1,
      "uniqueItems": true,
    ...
    "integrity": {
      "type": "object",
      "additionalProperties": false,
      "required": ["payload_digest", "source_producer_principal_ref"],
      "properties": {
        "payload_digest": {"$ref": "#/$defs/digest"},
        "source_producer_principal_ref": {"$ref": "#/$defs/stableId"},
...
- `effective_principal_id`, `actor_chain`, `ingest_time`, `ontology_release_digest`, `policy_bundle_digest` e `governed_context_digest` sono assegnati dal trusted ingress boundary e non sono asserzioni della sorgente. `integrity.source_producer_principal_ref` conserva separatamente il producer dichiarato; non concede Authority. Un producer incompatibile col Principal autenticato produce `PRINCIPAL_BINDING_MISMATCH`, quarantena e Audit Record. Nessun campo può essere sovrascritto dal payload. `event_time` resta un'asserzione della sorgente.
- L'envelope admitted realizza integralmente `GovernedContext` §1.4: la chiave di isolamento completa è costruibile per ogni evento ammesso e il digest è verificabile.
...
message InvocationContext {
  ...
  string purpose = 5;
  string ontology_release_digest = 6;
  ...
  string governed_context_digest = 16;
}
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "urn:ocor:schema:mcp-tool-contract:1.2",
  "type": "object",
  "additionalProperties": false,
  "required": [
    "contract_id",
  ...
  "properties": {
    ...
    "governed_context_ref": {"type": "string", "pattern": "^urn:sha256:[0-9a-f]{64}$"},
  ...
  "allOf": [
    ...
    },
    {
      ...
      "then": {
        "properties": {
          "approval": {
            "properties": {"required_approver_roles": {"minItems": 1}},
            "required": ["independent_approver_count", "required_approver_roles"]
          },
          "evidence_requirements": {
            "properties": {
              "input_evidence": {"const": "REQUIRED"},
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "urn:ocor:schema:action-type-contract:1.1",
  ...
  "properties": {
    ...
    "governed_context_requirements": {
      ...
      "required": [
        ...
        "principal_from_verified_transport",
        "actor_chain_from_verified_delegation",
        "mismatch_fail_closed"
      ],
      "properties": {
        "all_canonical_fields": {"const": true},
        "principal_from_verified_transport": {"const": true},
        "actor_chain_from_verified_delegation": {"const": true},
        "mismatch_fail_closed": {"const": true}
      }
    },
    ...
    "canonical_commit_binding": {
      ...
      "required": [
        ...
        "claim_source_binding_path",
        "decision_ref_path",
        "authority_ref_path",
        "evidence_set_path",
        "aggregate_ref_path",
        "expected_revision_path",
        "precondition_binding_refs",
        "invariant_binding_refs",
        "gate_package_digest_path",
      ...
      "properties": {
        ...
        "claim_source_binding_path": {"$ref": "#/$defs/jsonPointer"},
        "decision_ref_path": {"$ref": "#/$defs/jsonPointer"},
        "authority_ref_path": {"$ref": "#/$defs/jsonPointer"},
        "evidence_set_path": {"$ref": "#/$defs/jsonPointer"},
        "aggregate_ref_path": {"$ref": "#/$defs/jsonPointer"},
        "expected_revision_path": {"$ref": "#/$defs/jsonPointer"},
        "precondition_binding_refs": {
        ...
        "gate_package_digest_path": {"$ref": "#/$defs/jsonPointer"},
        "idempotency_key_path": {"$ref": "#/$defs/jsonPointer"}
      },
      "allOf": [
        {
          "if": {
  ...
  "allOf": [
    {
      ...
      "then": {
        "properties": {
          ...
          "risk_class": {"enum": ["R2_CONTROLLED", "R3_HIGH_IMPACT"]},
          "approval": {
            "properties": {
          ...
          "evidence_requirements": {
            "properties": {
              "proposal_evidence": {"const": "REQUIRED"},
              "minimum_count": {"minimum": 1}
            }
          },
          "compensation": {
    ...
    },
    {
      "if": {
        "properties": {
          "evidence_requirements": {
            "properties": {"proposal_evidence": {"const": "REQUIRED"}},
            "required": ["proposal_evidence"]
          }
        },
      ...
      "then": {
        "properties": {
          "evidence_requirements": {
            "properties": {"minimum_count": {"minimum": 1}}
          }
        }
      }
    },
...
`canonical_commit_binding` è obbligatorio se e solo se `target=CANONICAL_COMMIT` ed è vietato per `target=EXTERNAL_ADAPTER`. Oltre a ownership, aggregate type e classe prodotta, lega mediante JSON Pointer tipizzati Claim/source, Decision, Authority, Evidence set, aggregate ref, expected revision, GatePackage digest e idempotency key allo schema dei parameters; lega inoltre gli invariant e precondition ID. Un path assente, multiplo, non risolto o non compatibile col tipo atteso produce `CONTRACT_REJECTED`. Quando la classe è `CanonicalAssertion`, `requires_accepted_from_claim` e `requires_evidence_set` sono strutturalmente `true`.
...
- Ogni mutazione canonica ha risk floor `R2_CONTROLLED`, `approval.mode` umano, quorum e ruoli non vuoti, Decision/Authority/Evidence non vuoti e `minimum_count ≥ 1`; `R0`, `R1` e `NONE` sono strutturalmente impossibili.
...
- `C3` mappa deterministicamente precondition, invariant, revision e GatePackage mismatch a `PRECONDITION_FAILED`, `INVARIANT_VIOLATION`, `REVISION_CONFLICT`, `GATE_PACKAGE_MISMATCH`; nessun generico “reason code” è ammesso su questo path.
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "urn:ocor:schema:event-subscription-contract:1.1",
  ...
  "allOf": [
    {
      "if": {
        "properties": {
          "replay": {
            "properties": {"replay_allowed": {"const": true}},
            "required": ["replay_allowed"]
          }
        },
        "required": ["replay"]
      },
      "then": {"properties": {"replay": {"required": ["max_replay_window"]}}}
    },
    {
      "if": {
        "properties": {
      ...
      "then": {"properties": {"replay": {"not": {"required": ["max_replay_window"]}}}}
    }
  ],
  "$defs": {
    "stableId": {
...
Ogni evento consegnato risolve `governed_context_ref`, rivaluta Policy/Authority per delivery e conserva lo stesso context digest su replay; per gli eventi version-sensitive porta anche `canonical_commit` e `projection_watermark`. Replay è ammesso soltanto con finestra bounded esplicita e nuova autorizzazione; senza finestra produce `REPLAY_DENIED`. Il contratto non dichiara exactly-once e impone deduplica su `event_id`. Il cursor è opaco: nessun offset, topic o ID broker attraversa il contratto pubblico.
...
`ConflictScope` è un record chiuso `{ownership_boundary, aggregate_type_id, aggregate_ref, target_type, target_ref, effect_domain, resource_refs[], conflict_relation_version}`. La relazione di conflitto è deterministica e versionata; se lo scope o il footprint non sono calcolabili, la postura è il blocco conservativo dell'intero ownership boundary, non un allow implicito.

`IndeterminateEffectAdjudication` è append-only e distinto dalla FSM esecutiva: `{adjudication_id, action_instance_id, conflict_scope, adjudicator_principal_id, authority_ref, decision_ref, evidence_refs[1..*], disposition, residual_risk, created_at, signature_ref}` con `disposition ∈ {EFFECT_CONFIRMED, NO_EFFECT_CONFIRMED, RESIDUAL_RISK_ACCEPTED_FOR_NEW_ACTION}`. Solo Evidence positiva può scegliere i primi due esiti; il terzo non riscrive l'esito ignoto e autorizza una nuova proposta nominando il rischio. Un record valido ricalcola il blocco; nessuna modifica in-place o silenzio lo rimuove.
...
```mermaid
stateDiagram-v2
    [*] --> PROPOSAL_RECORDED: ACT-T01
    PROPOSAL_RECORDED --> PROPOSAL_RECORDED: ACT-T02
    [*] --> DENIED: ACT-T03
    PROPOSAL_RECORDED --> CONTROL_CHECK: ACT-T04
    CONTROL_CHECK --> DENIED: ACT-T05
    CONTROL_CHECK --> APPROVAL_PENDING: ACT-T06
    CONTROL_CHECK --> APPROVAL_RESOLVED: ACT-T07
    APPROVAL_PENDING --> APPROVAL_RESOLVED: ACT-T08
    APPROVAL_PENDING --> APPROVAL_REJECTED: ACT-T09a
    APPROVAL_PENDING --> APPROVAL_EXPIRED: ACT-T09b
    APPROVAL_RESOLVED --> DECISION_PENDING: ACT-T26
    DECISION_PENDING --> DECISION_REJECTED: ACT-T10a
    DECISION_PENDING --> INTENT_RECORDED: ACT-T10b
    INTENT_RECORDED --> PRE_DISPATCH_CHECK: ACT-T11
    PRE_DISPATCH_CHECK --> INVALIDATED: ACT-T13
    PRE_DISPATCH_CHECK --> CANCELLED: ACT-T23a
    PRE_DISPATCH_CHECK --> COMMAND_READY: ACT-T12
    COMMAND_READY --> CANCELLED: ACT-T23b
    COMMAND_READY --> DISPATCHED: ACT-T14
    COMMAND_READY --> DISPATCHED: ACT-T19
    COMMAND_READY --> CANONICAL_COMMIT_PENDING: ACT-T29
    DISPATCHED --> ACKNOWLEDGED: ACT-T15
    DISPATCHED --> EXECUTION_FAILED: ACT-T17a
    DISPATCHED --> EXECUTION_UNKNOWN: ACT-T18a
    CANONICAL_COMMIT_PENDING --> EXECUTION_CONFIRMED: ACT-T30
    CANONICAL_COMMIT_PENDING --> EXECUTION_FAILED: ACT-T31a
    CANONICAL_COMMIT_PENDING --> INVALIDATED: ACT-T31b
    ACKNOWLEDGED --> EXECUTION_CONFIRMED: ACT-T16
    ACKNOWLEDGED --> EXECUTION_FAILED: ACT-T17b
    ACKNOWLEDGED --> EXECUTION_UNKNOWN: ACT-T18b
    EXECUTION_UNKNOWN --> EXECUTION_CONFIRMED: ACT-T20a
    EXECUTION_UNKNOWN --> EXECUTION_FAILED: ACT-T20b
    EXECUTION_UNKNOWN --> EXECUTION_UNKNOWN: ACT-T20c
    EXECUTION_UNKNOWN --> EXECUTION_INDETERMINATE: ACT-T24
    EXECUTION_FAILED --> COMPENSATING: ACT-T21
    COMPENSATING --> COMPENSATED: ACT-T21a
    COMPENSATING --> COMPENSATION_FAILED: ACT-T21b
    COMPENSATING --> COMPENSATION_UNKNOWN: ACT-T21c
    COMPENSATION_UNKNOWN --> COMPENSATED: ACT-T21d
    COMPENSATION_UNKNOWN --> COMPENSATION_FAILED: ACT-T21e
    COMPENSATION_UNKNOWN --> COMPENSATION_INDETERMINATE: ACT-T25
    EXECUTION_CONFIRMED --> OUTCOME_PENDING: ACT-T27
    OUTCOME_PENDING --> OUTCOME_ASSESSED: ACT-T22
    OUTCOME_PENDING --> OUTCOME_UNOBSERVED: ACT-T28
```
...
| `G-FRESHNESS` | `expected_revision`, canonical commit, state revision, `governed_context_digest`, policy digest e finestra di validità, Authority, Approval set, ontology release, adapter binding e watermark coincidono con il `GatePackage` immutabile e con i valori correnti; nessun `STALE_CONTEXT`/`PROJECTION_NOT_READY`; nessun effetto irrisolto ha `ConflictScope` confliggente, salvo `IndeterminateEffectAdjudication` valido |
| `G-DISPATCH` | Emergency stop inattivo; `stop_epoch` invariato; `CapabilityLease` integro, non scaduto e legato a Principal/delegation/action/stop epoch; fencing token corrente; audit append durevole disponibile; circuit breaker chiuso; adapter conforme; idempotency key valida; command deadline non scaduta; command digest coincide col `GatePackage` |

`EMISSION-FENCE := G-FRESHNESS ∧ G-DISPATCH` è valutata atomicamente immediatamente prima di **ogni** invio esterno o interno, inclusi primo dispatch, retry e compensation. La valutazione e la registrazione del `DeliveryAttempt` appartengono alla stessa transazione locale del dispatcher. Qualunque drift materiale produce `INVALIDATED` prima dell'invio; un errore di valutazione non degrada a retry.

### 4.1.2 Transizioni normative

| Transition ID | Source | Evento/guardia | Effetto durevole | Destination |
|---|---|---|---|---|
...
`ACT-T23` ha precedenza su `ACT-T14`, `ACT-T19` e `ACT-T29`: uno stop accettato localmente prima dell'emissione impedisce il relay. A primo invio, retry e compensation, state revision, policy validity, Authority, Approval, release, watermark, stop epoch, `CapabilityLease`, audit, deadline, circuit breaker, adapter conformance e `GatePackage` sono letti in un unico snapshot di valutazione. Qualunque discrepanza materializza `INVALIDATED` prima del side effect.

Un `EXECUTION_UNKNOWN` o `COMPENSATION_UNKNOWN` non risolto entro il `reconciliation_deadline` transita in uno stato terminale che apre il workflow distinto `IndeterminateEffectAdjudication`. `G-FRESHNESS` blocca ogni proposta con `ConflictScope` confliggente; scope assente o ambiguo blocca l'ownership boundary. Solo un'adjudication firmata con Authority ed Evidence non vuota può ricalcolare il blocco. È questo controllo, non la sola idempotency key, a impedire doppia attuazione senza congelare azioni sicuramente indipendenti.
...
La compensation non è rollback implicito: è una nuova operazione governata con idempotency key, fencing, Delivery Attempt ed Execution Result propri. Immediatamente prima dell'invio rivalida atomicamente la stessa `EMISSION-FENCE`; drift o lease scaduta producono `INVALIDATED` e nessun invio. `COMPENSATION_UNKNOWN` non equivale a fallimento e non abilita retry ciechi. Il PoC esercita al massimo un percorso compensativo bounded mediante simulatore contract-faithful; il modello generalizzato multi-step `ELM-035` resta differito.
...
| `governed_context` | Record canonico §1.4 completo; include `domain_id`, marking ref, Principal transport-bound, actor chain, correlation e tutti i pin |
...
`ELM-070` rimane differito: il contratto di Counterfactual Query è riconosciuto e validabile, ma il profilo PoC restituisce `REJECTED{reason=CAPABILITY_DEFERRED, element=ELM-070}` per l'esecuzione completa abduction–action–prediction. Per eliminare la contraddizione con `FR-095` senza espandere il PoC, questa candidata propone `FR-095` P0/MVP fuori PoC. La proposta è `PROPOSED — AWAITING CHANGE CONTROL` e non modifica il registro sorgente; finché non approvata, l'acceptance PoC non può rivendicare `FR-095`. `do(X)` e il confronto interventistico non-AAP restano Design Target attivi.
...
Il lease è firmato dall'authority di capability; è monouso per `(action_instance_id, emission_attempt)` e non trasferibile. Principal, delegation, GCS, GatePackage o stop epoch non coincidenti, firma/revoca invalida o `expires_at ≤ now` producono `CAPABILITY_LEASE_EXPIRED`/`GOVERNED_CONTEXT_MISMATCH` e nessuna emissione.
...
L'operatore dipende dalla semantica dichiarata e il profilo supportato è totale. La candidata seleziona una sola algebra conservativa:

| Famiglia | Kind | Dominance | Join conservativo | Meet | Unknown / incomparabile |
|---|---|---|---|---|---|
| `classification_level` | `RESTRICTION` | reticolo `dominance_relation` | least upper bound univoco | greatest lower bound | `DENY`; oppure `top` solo se lo scheme lo dichiara esplicitamente |
| `mandatory_markings` | `RESTRICTION` | superset più restrittivo | unione | intersezione | `DENY` |
| `caveats` | `RESTRICTION` | superset più restrittivo | unione | intersezione | `DENY` |
| `dissemination_controls` | `RESTRICTION` | superset più restrittivo | unione | intersezione | `DENY` |
| `permitted_purposes` | `PERMISSION` | subset più restrittivo | intersezione | unione entro vocabolario | insieme vuoto ⇒ `DENY` |
| `handling_instructions` | `RESTRICTION` | superset più restrittivo | unione | intersezione | `DENY` |

Uno scheme che dichiari `PERMISSION` per caveat, mandatory marking, dissemination control o handling instruction è fuori dal profilo supportato e viene rifiutato in release admission. La tassonomia concreta resta governata da `OI-021`; il metamodello e gli operatori sono invece definiti qui.

$\bigsqcup$ è il join univoco del reticolo dichiarato. Il compilatore verifica chiusura, totalità nel profilo supportato, monotonia, determinismo, associatività, commutatività e idempotenza degli operatori. In assenza di scheme attivo, label nota, risultato univoco o contesto completo la richiesta è negata; nessun valore sconosciuto viene scartato. Declassification non usa il meet ordinario: richiede una transizione separata firmata da `declassification_authority_ref`, Human Gate, Evidence, before/after digest e nuovo artefatto. Il criterio «marking non decresce» (§7.2) è quindi calcolabile senza chiudere `OI-021`.
...
Della `CAP-024` il PoC copre la slice di isolamento, deployment ripetibile e recovery (`NFR-059`, `NFR-062`, `NFR-065`), la degradazione sicura sotto carico e guasto (`NFR-079`, §6.3) e le classi di workload asincrono (`NFR-072`, §4.2). `NFR-060`, `NFR-061`, `NFR-064`, `NFR-074` e `NFR-078` non sono inclusi nel profilo PoC ma restano requisiti `Confermato` nelle rispettive release sorgente; in particolare `NFR-078` è P0/MVP e non `Differito`. Il target indicativo `NFR-071` resta governato da `ASM-010` e `OI-008`. L'hardening esteso della capability è deferred rispetto al PoC; nessun requisito P0 con target **PoC** e obbligo `DEVE` è differito.
...
Ogni step del mission thread propaga il digest e, nel record governato policy-filtered, l'intero `GovernedContext`:
...
La matrice copre 183/196 decisioni su superfici subsystem/programme, 23/23 `ARC`, 18/18 `BR`, 174/174 `FR`, 93/93 `NFR`, 25/26 `CAP` — `CAP-026` è fuori perimetro — 103/103 `ELM`, 60/60 `RSK`. Le restanti 13 decisioni sono tracciate come `DOCUMENT_CONTROL` nella tabella successiva: coverage globale 196/196. Una allocazione programme/document non implica ownership runtime.
...
| Agent Kernel | `ARC-010`, `ARC-014–016` | `DEC-007–008`, `DEC-018`, `DEC-066`, `DEC-110–120`, `DEC-132–133`, `DEC-173` | `FR-038`, `FR-106–128`, `FR-140`, `FR-166–167`; `NFR-016`, `NFR-034–037`, `NFR-047–048` | `CAP-013–014`, `CAP-020`, `CAP-022` | `ELM-040`, `ELM-048`, `ELM-051`, `ELM-074–088`, `ELM-097`, `ELM-099`, `ELM-103` | `RSK-005`, `RSK-007`, `RSK-029–033`, `RSK-038` |
...
**Disposition tipizzata delle decisioni non-subsystem.** Le 13 decisioni assenti dalle righe subsystem sono allocate, non ignorate:

| Allocation kind | Decisioni | Superficie |
|---|---|---|
| `PROGRAMME_GOVERNANCE` | `DEC-037`, `DEC-059`, `DEC-068`, `DEC-088`, `DEC-098`, `DEC-109`, `DEC-121`, `DEC-149`, `DEC-171`, `DEC-193` | Programme & Evidence Governance; gate/chiusure IRB |
| `DOCUMENT_CONTROL` | `DEC-028`, `DEC-029`, `DEC-196` | apertura processo, delega gate e freeze della baseline |

I 17 `GATE-IRB` del Decision Register — `DEC-037`, `DEC-049`, `DEC-059`, `DEC-068`, `DEC-078`, `DEC-088`, `DEC-098`, `DEC-109`, `DEC-121`, `DEC-136`, `DEC-149`, `DEC-164`, `DEC-171`, `DEC-177`, `DEC-185`, `DEC-193`, `DEC-195` — hanno sempre allocation kind programme anche quando compaiono dentro un range largo di una riga subsystem. `DEC-194` è validation governance; `DEC-196` è document control. `DEC-173` e `DEC-175` sono invece sostantive e allocate rispettivamente ad Agent Kernel e Compiler & Gateway. Coverage: 183/196 nella matrice + 13/196 nella tabella = 196/196 globale. Nessun `DEC-*` è creato, modificato o chiuso.
...
| `CAP-024` hardening esteso | Separato dalla topologia PoC single-site/non-HA. In perimetro PoC: `NFR-059`, `NFR-062`, `NFR-065`, `NFR-072`, `NFR-079`. Fuori dal profilo PoC ma `Confermato` nelle release sorgente: `NFR-060`, `NFR-061`, `NFR-064`, `NFR-074`, `NFR-078`; target indicativo `NFR-071` governato da issue/assumption | Nessun claim HA, multi-site, scale nazionale o air-gap accreditation; nessun requisito P0/PoC con obbligo `DEVE` è differito |
...
La revisione v1.2 candidata non modifica questa conclusione. Correggere una contraddizione, completare un contratto o allocare un requisito non produce evidenza: `E1` resta `0`, `E2` resta `0`, i requisiti `Verified` restano `0`, le capability restano `Design Target` e le tecnologie `Candidate Implementation`. Gli emendamenti normativi elencati in §8 sono `PROPOSED — AWAITING CHANGE CONTROL` e non entrano in baseline finché la Architecture Review Authority non li approva secondo `DEC-196`.

# 8. Amendment Log v1.1 → v1.2 Candidate

Questa sezione registra l'intero delta della candidata rispetto alla baseline v1.1 identificata nel Document Control. I locator indicano le sezioni normative nelle quali ricade ciascun hunk del patch; le righe con `CC=Sì` sono soluzioni candidate tecnicamente determinate ma non approvate. L'approvazione, il rigetto o la modifica compete alla Architecture Review Authority. Nessuna riga eleva lo stato probatorio o modifica in-place i registri sorgente.

| ID candidato | Finding servito | Locator v1.2 | Modifica | Natura | Versione contratto | `CC` / stato |
|---|---|---|---|---|---|---|
| `V12-AM-01` | `DRF-015`; document control | Front matter, §7.3, §8 | Versione, derivazione e digest v1.1; stato candidato; authority ed evidence fence; nessuna approvazione implicita | Normativa di change control | — | No — controllo della candidata |
| `V12-AM-02` | `DRF-001`, `DRF-002`, `ARF-001` | §1.4, §3.0.1–§3.0.3, §3.2–§3.5, §4.1–§4.3, §5.1, §5.3, §6.3 | Record `GovernedContext` unico; origine `BODY`/`TRANSPORT`/`DERIVED_FROM_VERIFIED_BINDING`; equivalenza per digest; Principal effettivo solo da binding autenticato; mismatch fail-closed | Normativa | Ingestion 1.2; OpenAPI 1.2.0; MCP 1.2; Action 1.1; Event 1.1 | Sì — `PROPOSED — AWAITING CHANGE CONTROL` |
| `V12-AM-03` | `DRF-003`, `DRF-008`, `RV-01` | §2.5 r.10, §3.7, §4.1–§4.1.2 | `CANONICAL_COMMIT` con risk floor R2, Human Gate/Dual Control, quorum/ruoli, Decision, Authority, Evidence, claim/source, aggregate/revision, precondizioni/invarianti, idempotency e GatePackage; `approval.mode=NONE` vietato | Normativa | Action 1.1 | Sì — `PROPOSED — AWAITING CHANGE CONTROL` |
| `V12-AM-04` | `DRF-004`, `ARF-006` | §4.1–§4.1.2, §5.2, §5.5 | `EMISSION-FENCE = G-FRESHNESS ∧ G-DISPATCH` atomica per primo invio, retry e compensation; `CapabilityLease`, policy validity, watermark, stop epoch, audit, deadline, circuit breaker e GatePackage rivalidati; drift→`INVALIDATED` | Normativa | Action 1.1 | Sì — `PROPOSED — AWAITING CHANGE CONTROL` |
| `V12-AM-05` | `DRF-005`, `ARF-004`, `ARF-012` | §3.0.2, §3.1.1, §5.3 | `MarkingSchemeDefinition` calcolabile, totale sul profilo, monotono, deterministico e fail-closed; famiglie restriction/permission; unknown e incomparabili deny; declassification con Authority separata; `OI-021` resta aperta | Normativa | — | Sì — `PROPOSED — AWAITING CHANGE CONTROL` |
| `V12-AM-06` | `DRF-013`, `ARF-014` | §3.3 | `ObjectSnapshot` rende obbligatori relazioni, uncertainty, identity candidates ed explanation oltre a claims/evidence/provenance/marking | Normativa | OpenAPI 1.2.0 | No — correzione di completezza |
| `V12-AM-07` | `V12-RF-001`, `ARF-009` | §3.8 | Event contract porta GCS e rende `max_replay_window` obbligatoria se replay è ammesso e vietata altrimenti | Normativa | Event 1.1 | Sì — `PROPOSED — AWAITING CHANGE CONTROL` |
| `V12-AM-08` | `DRF-008`, `V12-RF-002`, `ARF-005` | §3.5, §3.7 | Quorum e ruoli non vuoti; evidence `minimum_count` coerente con `REQUIRED`; combinazioni target/effect e profilo `R2_CONTROLLED` fail-closed; reason code C3 nominati | Normativa | MCP 1.2; Action 1.1 | No — allineamento a guardie esistenti |
| `V12-AM-09` | `DRF-007`, `ARF-007` | §4.1–§4.1.2 | `ConflictScope` tipizzato e blocco conservativo; `IndeterminateEffectAdjudication` append-only con Authority/Evidence; nessun unblock per silenzio | Normativa | — | Sì — `PROPOSED — AWAITING CHANGE CONTROL` |
| `V12-AM-10` | `DRF-006`, `ARF-018` | §4.1–§4.1.2 | Diagramma e tabella riscritti in bijezione sulle tuple `(transition_id, source, destination)`; `ACT-T22` separato da `ACT-T27`; source/destination espliciti | Normativa/editoriale | — | No — correzione interna |
| `V12-AM-11` | `DRF-012`; `FR-095`/`ELM-070` | §1.5, §4.2, §6.1, §7.1 | Opzione conservativa selezionata: `ELM-070` resta differita e si propone di riclassificare `FR-095` fuori dal PoC; registri sorgente invariati | Normativa di scope | — | Sì — `PROPOSED — AWAITING CHANGE CONTROL` |
| `V12-AM-12` | `DRF-014`; scope fence | §1.5, §6.1, §7.1 | `NFR-078` resta `Confermato`, P0/MVP, fuori dal profilo PoC e non viene dichiarato differito; slice `CAP-024` corretta | Normativa/editoriale | — | No — ripristino del registro sorgente |
| `V12-AM-13` | `DRF-015`; traceability | §7.1 | `DEC-173` allocata ad Agent Kernel e `DEC-175` a Compiler & Gateway; coverage globale separa 183 decisioni subsystem/programme da 13 document/programme; `CAP-026` resta `OUT_OF_SCOPE` ma tracciata | Normativa | — | Sì per riallocazione — `PROPOSED — AWAITING CHANGE CONTROL` |
| `V12-AM-14` | `DRF-009`, `BA-01` | §2.6, §6.2, §8 | `BA-01` classificata `ARCHITECTURAL_ALTERNATIVE_REQUIRING_CHANGE_CONTROL`, `NO-GO` finché una sola alternativa, failure analysis e rollback non sono approvati | Normativa | — | Sì — `PROPOSED — AWAITING CHANGE CONTROL` |
| `V12-AM-15` | 15 `DRF-*`, 27 `ARF-*`, `RV-01/02` | §1–§8 | Riconciliazione finding, conteggi, locator, DAG `DRAFT-A`–`DRAFT-I`, versioni contrattuali e linguaggio probatorio resi coerenti; storico v1.0→v1.1 preservato in §8.3 | Documentale | Tutte come §8.2 | No per ledger; le singole proposte conservano il proprio `CC` |

Ogni modifica testuale v1.1→v1.2 ricade in almeno una riga precedente. Il controllo automatico del patch usa il primo heading normativo che precede ciascun hunk e verifica che tale sezione sia coperta dal campo Locator. Le modifiche meramente consequenziali — rinomina di versione, riferimenti incrociati, aggiornamento dei conteggi e allineamento terminologico — ereditano l'ID della modifica causale e non costituiscono decisioni autonome.

## 8.1 Difetti v1.1 riconciliati nella candidata

I 15 `DRF-*`, i 27 `ARF-*`, `RV-01`, `RV-02` e i nuovi `V12-RF-*` sono riconciliati nel Remediation Ledger esterno. `CLOSED_IN_CANDIDATE` significa soltanto che il difetto interno è corretto e testato nella candidata; `PENDING_CHANGE_CONTROL` significa che la soluzione tecnica è completa ma non è baseline. Nessuno stato chiude gli identificativi normativi `OI-*`, `ASM-*` o `RSK-*`.

## 8.2 Versioni dei contratti v1.1 → v1.2 Candidate

| Contratto | v1.1 | v1.2 Candidate | Motivo |
|---|---|---|---|
| `signed-canonical-ir` | `1.0` | `1.0` | Non modificato |
| `canonical-ingestion-envelope` | `1.1` | `1.2` | GCS completo, Principal effettivo e marking families |
| Named Query Gateway (OpenAPI) | `1.1.0` | `1.2.0` | GCS canonico e `ObjectSnapshot.required` completo |
| `FunctionRegistry`/`ModelRegistry` (Protobuf) | `ocor.registry.v1` | `ocor.registry.v1` | Estensione additiva del context; nessuna rinumerazione |
| `mcp-tool-contract` | `1.1` | `1.2` | GCS, quorum/ruoli ed evidence non vuota per execute |
| `action-type-contract` | `1.0` | `1.1` | Canonical binding, Human Gate, evidence e target/effect constraints |
| `event-subscription-contract` | `1.0` | `1.1` | GCS e replay window fail-closed |

## 8.3 Storico Amendment Log v1.0 → v1.1
...
### 8.3.1 Difetti introdotti nella v1.1 e riesaminati per la candidata
...
### 8.3.2 Versioni storiche dei contratti v1.0 → v1.1
...
## 8.4 Cosa questa revisione non fa
...
# Part II — DEC-208 Full Governed Agent Memory amendment

# OCOR Architectural Design Document v1.3 — Approved Amendment
...
| Element | `ELM-084 = CORE / P0 / PoC`, approved by `DEC-208` |
| Evidence state | unchanged; no implementation or production claim |
| Effective from | 2026-09-01 |

This approved amendment, ratified by `DEC-208`, modifies ADD v1.2 only through the replacements and additions below. The previous bounded-memory proposal is withdrawn.
...
## 6. Approval conditions satisfied

`DEC-208` promotes this amendment as part of the atomic ADD v1.3 baseline because:
...
The full-memory runtime remains `NO-GO`; this approval is a design-baseline disposition and does not claim implementation or Production readiness.
