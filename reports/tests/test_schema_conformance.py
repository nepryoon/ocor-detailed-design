#!/usr/bin/env python3
"""Conformance suite indipendente per i contratti formali dell'ADD v1.1.
...
ADD = ROOT / "inputs/normative/OCOR_Architectural_Design_Document_v1.1.md"
RESULTS = Path(__file__).with_name("conformance_results.json")
...
def marking():
    return {"scheme_id": urn("scheme"), "labels": ["SYNTHETIC"], "marking_digest": DIGEST}
...
def ingestion(mode="BATCH", semantic="OBSERVATION"):
    ...
    return {
        "envelope_version": "1.1", "event_id": UUID1,
        ...
        "integrity": {"payload_digest": DIGEST, "producer_principal_ref": urn("principal")},
        "actor_chain": [urn("principal")], "ontology_release_digest": DIGEST,
...
def mcp(effect="READ_ONLY"):
    return {
        "contract_id": urn("mcp"), "version": "1.0.0", "contract_digest": DIGEST,
...
def action(target="EXTERNAL_ADAPTER"):
    x = {
        "action_type_id": urn("action"), "version": "1.0.0", "contract_digest": DIGEST,
...
def event_contract():
    return {
        "subscription_contract_id": urn("subscription"), "version": "1.0.0", "contract_digest": DIGEST,
...
for mode, semantic in (("BATCH", "OBSERVATION"), ("STREAM", "OBSERVED_EVENT"), ("CDC", "CDC_CHANGE")):
    check("urn:ocor:schema:canonical-ingestion-envelope:1.1", f"positivo ingest_mode={mode}", ingestion(mode, semantic), "ACCEPT")
bad = ingestion("BATCH"); bad["transport"] = ingestion("STREAM", "OBSERVED_EVENT")["transport"]
check("urn:ocor:schema:canonical-ingestion-envelope:1.1", "negativo ramo BATCH: transport STREAM", bad, "REJECT")
bad = ingestion("STREAM", "OBSERVED_EVENT"); bad["transport"] = ingestion("BATCH")["transport"]
check("urn:ocor:schema:canonical-ingestion-envelope:1.1", "negativo ramo STREAM: transport BATCH", bad, "REJECT")
bad = ingestion("CDC", "OBSERVATION")
check("urn:ocor:schema:canonical-ingestion-envelope:1.1", "negativo ramo ingest CDC: semantic_class non CDC_CHANGE", bad, "REJECT")
bad = ingestion("CDC", "CDC_CHANGE"); bad["transport"] = ingestion("BATCH")["transport"]
check("urn:ocor:schema:canonical-ingestion-envelope:1.1", "negativo ramo ingest CDC: transport non CDC", bad, "REJECT")
bad = ingestion("STREAM", "CDC_CHANGE")
check("urn:ocor:schema:canonical-ingestion-envelope:1.1", "negativo ramo semantic CDC_CHANGE: ingest non CDC", bad, "REJECT")
for value in (NOW, None):
    x = ingestion(); x["effective_time"] = {"from": NOW, "to": value}
    check("urn:ocor:schema:canonical-ingestion-envelope:1.1", f"positivo oneOf effective_time.to={value!r}", x, "ACCEPT")
bad = ingestion(); bad["effective_time"] = {"from": NOW, "to": 7}
check("urn:ocor:schema:canonical-ingestion-envelope:1.1", "negativo oneOf effective_time.to", bad, "REJECT")
for value in (UUID1, None):
    x = ingestion(); x["causation_id"] = value
    check("urn:ocor:schema:canonical-ingestion-envelope:1.1", f"positivo oneOf causation_id={value!r}", x, "ACCEPT")
bad = ingestion(); bad["causation_id"] = 7
check("urn:ocor:schema:canonical-ingestion-envelope:1.1", "negativo oneOf causation_id", bad, "REJECT")
for field in ("before_digest", "after_digest"):
    for value in (DIGEST, None):
        x = ingestion("CDC", "CDC_CHANGE"); x["transport"][field] = value
        check("urn:ocor:schema:canonical-ingestion-envelope:1.1", f"positivo oneOf {field}={value!r}", x, "ACCEPT")
    bad = ingestion("CDC", "CDC_CHANGE"); bad["transport"][field] = 7
    check("urn:ocor:schema:canonical-ingestion-envelope:1.1", f"negativo oneOf {field}", bad, "REJECT")

# MCP: gli 11 rami if/then (incluso retry annidato).
x = mcp(); x["retry"] = {"retry_safe": False, "max_attempts": 1}
check("urn:ocor:schema:mcp-tool-contract:1.1", "positivo ramo retry_safe=false", x, "ACCEPT")
bad = copy.deepcopy(x); bad["retry"]["max_attempts"] = 2
check("urn:ocor:schema:mcp-tool-contract:1.1", "negativo ramo retry_safe=false", bad, "REJECT")
x = mcp(); x["risk_class"] = "HIGH"; x["approval"] = approval("DUAL_CONTROL", 2, [urn("r1"), urn("r2")])
check("urn:ocor:schema:mcp-tool-contract:1.1", "positivo ramo HIGH/CRITICAL", x, "ACCEPT")
bad = copy.deepcopy(x); bad["approval"]["independent_approver_count"] = 1
check("urn:ocor:schema:mcp-tool-contract:1.1", "negativo ramo HIGH/CRITICAL quorum", bad, "REJECT")
x = mcp("EXECUTE_APPROVED_ACTION"); x["allowed_autonomy_tiers"] = ["EXECUTE_APPROVED"]; x["approval"] = approval("HUMAN_GATE", 1, [urn("approver")]); x["error_codes"].append("EXECUTION_UNKNOWN")
check("urn:ocor:schema:mcp-tool-contract:1.1", "positivo ramo EXECUTE_APPROVED_ACTION (vincoli 1)", x, "ACCEPT")
check("urn:ocor:schema:mcp-tool-contract:1.1", "positivo ramo EXECUTE_APPROVED_ACTION (vincoli 2)", x, "ACCEPT")
for label, mutate in (
    ...
    check("urn:ocor:schema:mcp-tool-contract:1.1", f"negativo EXECUTE: {label}", bad, "REJECT")
x = mcp(); x["approval"] = approval("DUAL_CONTROL", 2, [urn("r1"), urn("r2")])
check("urn:ocor:schema:mcp-tool-contract:1.1", "positivo ramo DUAL_CONTROL", x, "ACCEPT")
bad = copy.deepcopy(x); bad["approval"]["independent_approver_count"] = 1
check("urn:ocor:schema:mcp-tool-contract:1.1", "negativo ramo DUAL_CONTROL", bad, "REJECT")
x = mcp(); x["compensation"] = {"mode": "HUMAN_AUTHORIZED", "irreversibility_class": "COMPENSATABLE", "compensation_contract_id": urn("comp")}
check("urn:ocor:schema:mcp-tool-contract:1.1", "positivo ramo compensation HUMAN_AUTHORIZED", x, "ACCEPT")
bad = copy.deepcopy(x); del bad["compensation"]["compensation_contract_id"]
check("urn:ocor:schema:mcp-tool-contract:1.1", "negativo ramo compensation HUMAN_AUTHORIZED", bad, "REJECT")
x = mcp(); x["approval"] = approval("HUMAN_GATE", 1, [urn("approver")])
check("urn:ocor:schema:mcp-tool-contract:1.1", "positivo ramo HUMAN_GATE", x, "ACCEPT")
bad = copy.deepcopy(x); del bad["approval"]["required_approver_roles"]
check("urn:ocor:schema:mcp-tool-contract:1.1", "negativo ramo HUMAN_GATE", bad, "REJECT")
for effect, tier in (("PROPOSE_ACTION", "PROPOSE"), ("SIMULATE", "SIMULATE"), ("ANALYZE", "ANALYZE")):
    x = mcp(effect); x["allowed_autonomy_tiers"] = [tier]
    check("urn:ocor:schema:mcp-tool-contract:1.1", f"positivo ramo {effect}", x, "ACCEPT")
    bad = copy.deepcopy(x); bad["allowed_autonomy_tiers"] = ["OBSERVE"]
    check("urn:ocor:schema:mcp-tool-contract:1.1", f"negativo ramo {effect}", bad, "REJECT")
check("urn:ocor:schema:mcp-tool-contract:1.1", "positivo ramo READ_ONLY", mcp(), "ACCEPT")
bad = mcp(); bad["allowed_autonomy_tiers"].append("EXECUTE_APPROVED")
check("urn:ocor:schema:mcp-tool-contract:1.1", "negativo ramo READ_ONLY con execute", bad, "REJECT")

# Action Type: i 9 rami if/then (inclusi due annidati).
x = action("CANONICAL_COMMIT")
x["risk_class"] = "R2_CONTROLLED"
x["approval"] = approval("HUMAN_GATE", 1, [urn("canonical-approver")])
check("urn:ocor:schema:action-type-contract:1.0", "positivo ramo target CANONICAL_COMMIT", x, "ACCEPT")
check("urn:ocor:schema:action-type-contract:1.0", "positivo ramo produced CanonicalAssertion", x, "ACCEPT")
for label, mutate in (
    ...
    bad = copy.deepcopy(x); mutate(bad)
    check("urn:ocor:schema:action-type-contract:1.0", f"negativo canonical: {label}", bad, "REJECT")
bad = action("CANONICAL_COMMIT")
check(
    "urn:ocor:schema:action-type-contract:1.0",
    "negativo semantico: CANONICAL_COMMIT R1 senza Human Gate",
    bad,
    "REJECT",
)
x = action()
check("urn:ocor:schema:action-type-contract:1.0", "positivo ramo target EXTERNAL_ADAPTER", x, "ACCEPT")
bad = copy.deepcopy(x); bad["canonical_commit_binding"] = action("CANONICAL_COMMIT")["canonical_commit_binding"]
check("urn:ocor:schema:action-type-contract:1.0", "negativo external con canonical binding", bad, "REJECT")
x = action(); x["risk_class"] = "R3_HIGH_IMPACT"; x["approval"] = approval("DUAL_CONTROL", 2, [urn("r1"), urn("r2")])
check("urn:ocor:schema:action-type-contract:1.0", "positivo ramo R3_HIGH_IMPACT", x, "ACCEPT")
bad = copy.deepcopy(x); bad["approval"] = approval("HUMAN_GATE", 1, [urn("r1")])
check("urn:ocor:schema:action-type-contract:1.0", "negativo ramo R3_HIGH_IMPACT", bad, "REJECT")
x = action(); x["risk_class"] = "R2_CONTROLLED"; x["approval"] = approval("HUMAN_GATE", 1, [urn("r1")])
check("urn:ocor:schema:action-type-contract:1.0", "positivo ramo R2_CONTROLLED", x, "ACCEPT")
bad = copy.deepcopy(x); bad["approval"] = approval()
check("urn:ocor:schema:action-type-contract:1.0", "negativo ramo R2_CONTROLLED", bad, "REJECT")
x = action(); x["compensation"] = {"mode": "HUMAN_AUTHORIZED", "irreversibility_class": "COMPENSATABLE", "compensation_action_type_id": urn("comp")}
check("urn:ocor:schema:action-type-contract:1.0", "positivo ramo compensation HUMAN_AUTHORIZED", x, "ACCEPT")
bad = copy.deepcopy(x); del bad["compensation"]["compensation_action_type_id"]
check("urn:ocor:schema:action-type-contract:1.0", "negativo ramo compensation HUMAN_AUTHORIZED", bad, "REJECT")
...
check("urn:ocor:schema:action-type-contract:1.0", "positivo ramo IRREVERSIBLE", x, "ACCEPT")
bad = copy.deepcopy(x); bad["risk_class"] = "R2_CONTROLLED"; bad["approval"] = approval("HUMAN_GATE", 1, [urn("r1")])
check("urn:ocor:schema:action-type-contract:1.0", "negativo ramo IRREVERSIBLE risk R2", bad, "REJECT")
x = action(); x["retry"] = {"retry_safe": False, "max_attempts": 1}; x["error_codes"].append("EXECUTION_UNKNOWN")
check("urn:ocor:schema:action-type-contract:1.0", "positivo ramo retry_safe=false annidato", x, "ACCEPT")
check("urn:ocor:schema:action-type-contract:1.0", "positivo ramo retry_safe=false root", x, "ACCEPT")
bad = copy.deepcopy(x); bad["retry"]["max_attempts"] = 2
check("urn:ocor:schema:action-type-contract:1.0", "negativo retry_safe=false max_attempts", bad, "REJECT")
bad = copy.deepcopy(x); bad["error_codes"] = ["POLICY_DENIED"]
check("urn:ocor:schema:action-type-contract:1.0", "negativo retry_safe=false error code", bad, "REJECT")

# Event subscription: schema senza if/then, ma con positivi e negativi strutturali.
x = event_contract()
check("urn:ocor:schema:event-subscription-contract:1.0", "baseline positivo", x, "ACCEPT")
for label, mutate in (
    ...
    bad = copy.deepcopy(x); mutate(bad)
    check("urn:ocor:schema:event-subscription-contract:1.0", f"negativo {label}", bad, "REJECT")
...
def qctx(mode="BEST_AVAILABLE"):
    c = {
        ...
        "purpose": "test", "ontology_release": "1.0.0", "policy_bundle_digest": DIGEST,
...
for name, instance in requests.items():
    check("OpenAPI 1.1.0", f"positivo request {name}", instance, "ACCEPT", oas_schema(name))
    bad = copy.deepcopy(instance); del bad[next(iter(bad))]
    check("OpenAPI 1.1.0", f"negativo required request {name}", bad, "REJECT", oas_schema(name))

for mode in ("AT_LEAST_COMMIT", "EXACT_AT_COMMIT"):
    check("OpenAPI 1.1.0", f"positivo ramo Consistency {mode}", qctx(mode)["consistency"], "ACCEPT", oas_schema("Consistency"))
bad = {"mode": "EXACT_AT_COMMIT"}
check("OpenAPI 1.1.0", "negativo ramo Consistency senza required_commit", bad, "REJECT", oas_schema("Consistency"))
check("OpenAPI 1.1.0", "positivo Consistency BEST_AVAILABLE senza commit", {"mode": "BEST_AVAILABLE"}, "ACCEPT", oas_schema("Consistency"))
...
check("OpenAPI 1.1.0", "positivo ramo Problem version-sensitive", problem, "ACCEPT", oas_schema("Problem"))
bad = copy.deepcopy(problem); del bad["served"]
check("OpenAPI 1.1.0", "negativo ramo Problem version-sensitive", bad, "REJECT", oas_schema("Problem"))
...
check("OpenAPI 1.1.0", "positivo ramo Problem CAPABILITY_DEFERRED", problem2, "ACCEPT", oas_schema("Problem"))
bad = copy.deepcopy(problem2); del bad["deferred_element"]
check("OpenAPI 1.1.0", "negativo ramo Problem CAPABILITY_DEFERRED", bad, "REJECT", oas_schema("Problem"))
for name, base, field in (
    ...
    for value in values:
        x = {**base, field: value}
        check("OpenAPI 1.1.0", f"positivo oneOf {name}.{field}={value!r}", x, "ACCEPT", oas_schema(name))
    bad = {**base, field: 7}
    check("OpenAPI 1.1.0", f"negativo oneOf {name}.{field}", bad, "REJECT", oas_schema(name))
...
if_counts["OpenAPI 1.1.0"] = count_ifs(openapi)
expected = {
    "urn:ocor:schema:signed-canonical-ir:1.0": 0,
    "urn:ocor:schema:canonical-ingestion-envelope:1.1": 4,
    "urn:ocor:schema:mcp-tool-contract:1.1": 11,
    "urn:ocor:schema:action-type-contract:1.0": 9,
    "urn:ocor:schema:event-subscription-contract:1.0": 0,
    "OpenAPI 1.1.0": 3,
}
